...
public class ApplicationDbContextInitializer
{
    ...
    /// </summary>
    /// <returns></returns>
    public async Task InitialiseUsersAsync() 
    {
        ...
        users = new List<User>()
        {
            new User()
            {
                ...
                ProfilePictureDataUrl = $"{_optSystemSettings.Value.HostDomainName}/Image/2.png",
                PasswordHash = user.CreatePassword("123456")
            },
            new User()
            {
                Id = 2,
    ...
    /// </summary>
    /// <returns></returns>
    public async Task InitialiseRolesAsync()
    {
    ...
    /// </summary>
    /// <returns></returns>
    public async Task InitialiseMenusAsync() 
    {
    ...
    /// </summary>
    /// <returns></returns>
    public async Task InitialisePermissionAsync() 
    {
    ...
    /// </summary>
    /// <returns></returns>
    public async Task InitialiseRoleMenusAsync() 
    {
        var menus = await _context.Menus.ToListAsync();
        if (!menus.Any()) return;

        var roleMenus = await _context.RoleMenus.ToListAsync();
        if (roleMenus.Any()) return;

        var administratorRoleMenus = menus.Where(x => x.Meta.Title == "首页" || x.Meta.Title == "配置");
        foreach (var menu in administratorRoleMenus)
        {
            ...
            var childrenMenus = GetMenuAllChildren(menu.Id, menus);
        ...
        var commonRoleMenus = menus.Where(x => x.Meta.Title == "首页");
        foreach (var menu in commonRoleMenus)
        {
            await _context.RoleMenus.AddAsync(new RoleMenu()
            {
                Id = _snowFlakeService.GenerateId(),
        ...
        var devRoleMenus = menus;
        foreach (var menu in devRoleMenus)
        {
            await _context.RoleMenus.AddAsync(new RoleMenu()
            {
                Id = _snowFlakeService.GenerateId(),
    ...
    /// <returns></returns>
    public List<Menu> GetMenuAllChildren(long id, List<Menu> menus) 
    {
        var query = menus.Where(x => x.ParentId == id).ToList();
        return query.Concat(query.SelectMany(sm => GetMenuAllChildren(sm.Id, menus))).ToList();
    }
